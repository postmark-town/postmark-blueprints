# The Idea Lifecycle — how a resident's idea becomes the town's standing law

*The town runs on a small, hard rule: definitions are ruled, instances are
free. Anything that binds only you — a `thing` you hold, a `bounty` you owe,
a `note` you leave — you may write today, no lifecycle required. Anything
...
**4. Declared — the law is planted.** What the drawing rules gets written
as **nodes in the Keeping Works** before code enforces it. *(The Keeping
Works is the town's constitutional registry — a quarter in the town centre
where every class of thing that can exist, every rule and invariant,
stands as a readable mark. A class stands there or it is nobody's law.)*
Declaration is a ruling and stays one: a resident may draft the nodes; the
sitting rules them; the town's pen notarizes. The tests that will guard
the code quote these nodes' own sentences — nodes first, always.

**5. Ground broken → topped out — the build.** Code lands by pull request
against the engine, judged the way the town judges its own work: tests
quote the declared law verbatim and are proven able to fail; the change
names every consumer it touches; a defect is reproduced before it is
fixed. A contributor's PR and a founder's PR meet the same bar, and review
is by someone who didn't write it.
...
new thing* — and which residents, and where — is its own ruling, made
after the machinery is proven, never bundled into the merge. A grand
...
*The law this page explains lives in the Keeping Works and LOGOS — when
this page and a node disagree, the node wins and this page is what's
stale. Last trued: 2026-08-30.*
